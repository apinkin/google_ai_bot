...
class MyBot(BaseBot):
    ...
    def enemy_ships_attacking(self, planet):
        return sum( [ fleet.ship_count for fleet in planet.attacking_fleets if fleet.owner in player.NOT_ME ] )

    def enemy_ships_reinforcing(self, planet):
        return sum( [ fleet.ship_count for fleet in planet.reinforcement_fleets if fleet.owner in player.NOT_ME ] )
    ...
    def is_defendable(self, planet_to_defend, max_distance, ship_deficit):
        my_nearest_planets = sorted(self.universe.my_planets, key=lambda p : p.distance(planet_to_defend) + p.id/1000000.0)
        ships_can_send = 0
        for source in my_nearest_planets:
            if source.id == planet_to_defend.id:
                continue
            if source.distance(planet_to_defend) > max_distance:
                break
            ships_can_send += self.ships_available[source]
            if ships_can_send >= ship_deficit:
                return True
        return False
            ...
            for source in my_nearest_planets:
                ...
                if planet_to_attack_future.owner != player.ME:
                    if planet_to_attack.owner == player.NOBODY:
                        ...
                        if current_ships_needed <= self.ships_available[source]:
                            if planet_to_attack.growth_rate >= source.growth_rate:
                                current_ships_needed = self.ships_available[source]
                            source.send_fleet(planet_to_attack, current_ships_needed)
                            self.ships_available[source] -= current_ships_needed
                            break

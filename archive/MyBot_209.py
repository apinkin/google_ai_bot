...
from planetwars.universe2 import Universe2, copy
...
#                if planet_to_attack.id == 5 and distance == 15:
#                    log.info("get avail from %s = %s" % (planet, ships_avail))
                result += ships_avail
        return result

    def is_safe_to_attack(self, planet_to_attack, player, distance, ships_to_send):
            ...
            if planet.id != planet_to_attack.id and planet.distance(planet_to_attack) <= distance and self.ships_needed[planet] == 0:
                ...
                if ships_avail > 0:
                    for turn in range(1, distance + 1):
                        enemy_max_aid = self.max_aid_at_turn[PLAYER2][planet][turn]
                        my_max_aid = self.max_aid_at_turn[PLAYER1][planet][turn] + self.planet_timeline[planet][turn-1][1] - ships_to_send
                        if enemy_max_aid > my_max_aid:
                            return False
                ships_left_to_send -= ships_avail
    ...
    def doPrep(self):
        ...
        self.total_growth_rate_future = {PLAYER1:0, PLAYER2:0}
        ...
        for planet in self.all_planets:
            ...
            need_help = False
#            if planet.id == 4:
#                log.info("timeline for %s: %s, scheduled: %s" % (planet, self.planet_timeline[planet], scheduled_fleets_to_planet))
            ...
            if not need_help:
                ...
                for step in reversed(self.planet_timeline[planet]):
                    ...
                    step_index -= 1
                self.ships_available_at_turn[planet][0] = min(planet.ship_count, self.ships_available_at_turn[planet][1] - planet.growth_rate)
            else:
            ...
            planet_final_state = self.planet_timeline[planet][self.max_distance_between_planets + 20 - 1]
            if planet_final_state[0] != NOBODY:
                self.total_growth_rate_future[planet_final_state[0]] += planet.growth_rate
...
#                    continue
    ...
    def doFirstTurnOffense(self):
        ...
        profits = []
        for c in candidates:
            attack_score = (self.max_distance_between_planets - c.distance(self.my_home) + HORIZON_FIRST) * c.growth_rate
...
#                        break
                    ...
                    #log.info("aids for %s: enemy %s , me %s" % (planet_to_attack, enemy_max_aid, my_max_aid))
                    ...
                    if ships_avail_to_attack >= ships_to_send:
                        ...
                        #growth_rate_diff = self.total_growth_rate[PLAYER1] - self.total_growth_rate[PLAYER2]
                        growth_rate_diff = self.total_growth_rate[PLAYER1] - self.total_growth_rate_future[PLAYER2]
                        total_ships_diff = self.total_ships[PLAYER1] - self.total_ships[PLAYER2]
                        should_attack_neutrals = (growth_rate_diff <= 0)
                        #should_attack_neutrals = True
                        ...
                        #if planet_to_attack_future_owner in player.ENEMIES or (self.is_safe_to_attack(planet_to_attack, PLAYER1, distance, ships_to_send) and (attack_score-cost_to_conquer) >= ATTACK_SCORE_THRESHOLD):
                        if planet_to_attack_future_owner in player.ENEMIES or (should_attack_neutrals and (attack_score-cost_to_conquer) >= ATTACK_SCORE_THRESHOLD):
                        #if planet_to_attack_future_owner in player.ENEMIES or (attack_score-cost_to_conquer) >= ATTACK_SCORE_THRESHOLD or ((attack_score-cost_to_conquer) >= 35 and self.is_safe_to_attack(planet_to_attack, PLAYER1, distance, ships_to_send)):

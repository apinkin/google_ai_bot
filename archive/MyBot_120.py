...
class MyBot(BaseBot):
    ...
    def doPrep(self):
        ...
        for planet in self.all_planets:
            ...
            for turn in range(1, self.max_distance_between_planets+2):
                max_aid = 0
                #for my_planet in self.my_planets:
...
#                    #max_aid = self.ships_available[enemy_planet]
...
#                    continue
                ...
                if planet_to_attack_future_owner != player.ME and ships_to_send > 0 and ships_to_send <= self.ships_available[my_planet]:
                    ...
                    #attack_score += self.get_sum_not_my_growth_within_dist(planet_to_attack, self.max_distance_between_planets/4)
                    if planet_to_attack_future_owner in player.ENEMIES:
                        attack_score *= 2
        ...
        if self.universe.game.turn_count == 1:
            ...
            for c in candidates:
                ...
                attack_score = (self.max_distance_between_planets - c.distance(my_home) + 40) * c.growth_rate - weight

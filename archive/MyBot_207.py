...
#                log.info("timeline for %s: %s, scheduled: %s" % (planet, self.planet_timeline[planet], scheduled_fleets_to_planet))
    ...
    def doFirstTurnOffense(self):
        ...
        profits = []
        for c in candidates:
            attack_score = (self.max_distance_between_planets - c.distance(self.my_home) + 40) * c.growth_rate
                ...
                for distance in range(min_distance, max_distance+1):
                    ...
                    if planet_to_attack_future_owner == player.NOBODY:
                        ...
                        if planet_to_attack_future_owner == NOBODY and self.enemy_com.distance(planet_to_attack) < distance:
                            break
                    #log.info("Time to profit for %s is %s, distance %s" % (planet_to_attack, time_to_profit, distance))

    ...
    def doDefenseOffense(self):
        ...
        for my_planet in self.universe.my_planets:
            for planet_to_attack in self.universe.all_planets:
                ...
                my_max_aid = max(0, self.my_max_aid_at_turn[planet_to_attack][attack_distance+time_to_profit] - ships_to_send)
                if my_max_aid > 0:
                    log.info("positive my_max_aid: %s" % my_max_aid)
                if planet_to_attack_future_owner != player.ME and ships_to_send > 0 and (ships_to_send-my_max_aid) <= self.ships_available[my_planet]:
